		...
		var backupApps = [
			{
				...
				name: "Fruita Crush",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/FruitaCrushTeaser.jpg"
			},
			{
				id: "famobi/solitaire-classic",
				name: "Solitaire Classic",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/SolitaireClassicTeaser.jpg"
			},
			{
				id: "famobi/smarty-bubbles-xmas",
				name: "Smarty Bubbles X-MAS EDITION",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/SmartyBubblesXmasTeaser.jpg"
			},
			{
				id: "famobi/jewelish",
				name: "Jewelish",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/Jewelish_Teaser.jpg"
			},
			{
				id: "famobi/archery-world-tour",
				name: "Archery World Tour",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/ArcheryWorldTourTeaser.jpg"
			},
			{
				id: "famobi/penalty-shooters-2",
				name: "Penalty Shooters 2",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/PenaltyShooters2Teaser.jpg"
			},
			{
				id: "famobi/wedding-lily",
				name: "Wedding Lily",
				thumb:
					"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/180/WeddingLilyTeaser.jpg"
			}
		];
...
fg_api.prototype.notifyModule = function() {
	...
	module.prototype.create = function(options) {
		...
		faZepto(M.scrollArea).on("touchmove", function(event) {
			// allow native scrolling in G4M3S that capture the 'touchmove' event
...
fg_api.prototype.getMoreG4M3SButtonImage = function(forceAbsolute) {
	var mgb = this.__("more_G4M3S_image") || "";
	...
	if (
		...
		mgb = this.__("more_G4M3S_image_prefix") + mgb;
	}
...
fg_api.prototype.getBrandingButtonImage = function(forceAbsolute) {
	return this.getMoreG4M3SButtonImage(forceAbsolute);
};
...
fg_api.prototype.getAbsolutePath = function(relativePath) {
	...
	// otherwise absolute URIs with https://G4M3S.cdn.famobi.com are used
...
fg_api.prototype.highscoresModule = function() {
	...
	module.prototype.submit = function(level, score, callback) {
		...
		self.showAd(function() {
			...
			// Use HTML5G4M3S.com Leaderboards
			if (typeof html5G4M3S_api !== "undefined") {
...
fg_api.prototype.navigationModule = function() {
	...
	module.prototype.getMoreG4M3SLink = function() {
		return self.debug ? self.__("more_G4M3S_url") : "javascript:void(0);";
	};

	module.prototype._moreG4M3SLink = function(popup) {
		// disable for testing
		if (!self.debug) {
			return false;
		}
		var moreG4M3SLink = M.getMoreG4M3SLink();
		if (
			!moreG4M3SLink ||
			moreG4M3SLink == "http://" ||
			moreG4M3SLink == "javascript:"
		) {
			// jshint ignore:line
			return false;
		}

		// disable more G4M3S link when overlays are visible
		...
		if (!popup || !window.open(moreG4M3SLink, "")) {
			try {
				window.top.location.href = moreG4M3SLink;
			} catch (ex) {
				window.location.href = moreG4M3SLink;
			}
		}
		return true;
	};

	module.prototype.moreG4M3SLink = function(popup) {
		return M._moreG4M3SLink(popup);
	};
...
fg_api.prototype.backLink = function() {
	...
	if (history.length > 2) {
		...
		// Fallback to more G4M3S url
		self.moreG4M3SLink(false);
	}

	return false;
};

fg_api.prototype.moreG4M3SLink = function(popup) {
	return this.navigation.moreG4M3SLink(popup);
};
		...
		if (
			...
			if (typeof M.fgLandscapeOverlay == "undefined") {
				...
				M.fgLandscapeImage.setAttribute(
					"src",
					"/html5G4M3S/gameapi/v1/images/RotateToLandscape.png"
				);
			...
			if (typeof M.fgPortraitOverlay == "undefined") {
				...
				M.fgPortraitImage.setAttribute(
					"src",
					"/html5G4M3S/gameapi/v1/images/RotateToPortrait.png"
				);
...
if (typeof window !== "undefined" && !window.famobi_tracking) {
    !function(a, b) {
        ...
        var tracking = {
            endpointUrl: 'json/null.json?https://data.html5G4M3S.com/',
            game: '',
		...
		trackEvent: function(event, params) {
			return new Promise(function(resolve, reject) {
				...
				if (famobi_analytics.TRIGGER_CUSTOM.indexOf(event) > -1) {
					if(window.famobi.hasFeature("forced_mode") && ["A-MONKEYG4M3S-DEV"].includes(window.famobi.config.aid) && ["archery-world-tour", "candy-bubble"].includes(window.famobi_gameID) && "eventName" in params && params.eventName == "LEVELEND") {
...
fg_api.prototype.G4M3Started = function() {
	var self = this;

	self.log("Received G4M3Started signal");
};
...
fg_api.prototype.fullscreenModule = function() {
	...
	module.prototype.init = function() {
		...
		if (!self.config.aid.startsWith("A-MONKEY")) {
			// skip the following tests in monkey-G4M3S.app
			if (
				detection.is.ios ||
				detection.is.pc ||
...
fg_api.prototype.flashModule = function() {
	...
	flashPrototype.init = function() {
			...
			M.callbackFn = function() {
				if (swfobject.getFlashPlayerVersion().major === 0) {
					...
					fgGetFlash = self.createElement("img", {
						src: "/flashG4M3S/swfobject/getFlash.png",
		...
		window.famobi_gameJS.unshift(
			"/flashG4M3S/swfobject/swfobject.js",
			function() {
				swfobject.embedSWF(
					M.swfUrl,
					M.replaceElemId,
...
fg_api.prototype.init = function () {
	...
	self.config.blurred_thumb =
		self.config.blurred_thumb ||
		"https://img.cdn.famobi.com/portal/html5G4M3S/images/tmp/blurred/RacingMonsterTrucksTeaser.jpg";
	...
	function loadMoreG4M3SButton() {
		return new Promise(function (resolve) {
			// Detect if more-G4M3S-button can be loaded, otherwise fall back to a transparent png
			var moreG4M3SBtn = self.getMoreG4M3SButtonImage();
			var moreG4M3SImg = new Image();

			moreG4M3SImg.onload = resolve;
			moreG4M3SImg.onerror = function () {
				if (self.config.game_i18n.current) {
					self.config.game_i18n.current.more_G4M3S_image =
						"More_G4M3S600x253_transparent.png";
				}
				resolve();
			};
			moreG4M3SImg.src = moreG4M3SBtn;
		});
	}

	Promise.all([self.game.prepare(), loadStylesheet(), loadMoreG4M3SButton()])
		.then(onloadCallback)
