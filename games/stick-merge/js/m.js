...
class Preloader extends Phaser.Scene {
    ...
    initResize() {
        ...
        var G4M3Size = this.scale.G4M3Size;
        var width = G4M3Size.width;
        var height = G4M3Size.height;
    ...
    updateCamera() {
        ...
        if (window.innerHeight < MainGame.Config.MAX_HEIGHT) {
            ...
            sdvigY = this.scale.G4M3Size.height - window.innerHeight
        }
...
GameGUI = function(link) {
    this.G4M3Screen = link;
    this.layerButtons = this.G4M3Screen.layerButtons
};
GameGUI.prototype.init = function() {
    _this = this.G4M3Screen;
...
GameGUI.prototype.clickSelectGunPrev = function() {
    ...
    if (this.selectedGun < 1) {
        this.selectedGun = this.G4M3Screen.MAX_TYPES_CAR
    }
    this.updateSelectGun()
};
GameGUI.prototype.clickSelectGunNext = function() {
    this.selectedGun++;
    if (this.selectedGun > this.G4M3Screen.MAX_TYPES_CAR) {
        this.selectedGun = 1
    }
    this.updateSelectGun()
};
...
GameGUI.prototype.updateTweeningHat = function() {
    ...
    if (this.stepTweeningHat == this.arTweeningHat.length) {
        ...
        if (this.stepTweeningHat < 10) {
            this.G4M3Screen.time.delayedCall(100, this.updateTweeningHat, [], this)
        } else {
            var delay = 100 + 50 * (this.stepTweeningHat - 10);
            this.G4M3Screen.time.delayedCall(delay, this.updateTweeningHat, [], this)
        }
    }
};
...
GameGUI.prototype.checkFortunaWheelWindow = function() {
    if (this.G4M3Screen.freeTimeWheel > 0) {
...
GameGUI.prototype.updateFortunaWheelWindow = function(countDownNextFree) {
    var str1 = MainGame.GAME_TEXT.free_time + " " + this.G4M3Screen.freeTimeWheel + "/" + MainGame.maxTimeWheel;
    ...
    if (this.G4M3Screen.freeTimeWheel == MainGame.maxTimeWheel) {
...
GameGUI.prototype.closeRewardWindow = function() {
    ...
    this.updateFortunaWheelWindow(_this.countDownNextFree);
    this.eventFonImputUp = this.closeFortunaWheelWindow;
    if (MainGame.reward_wheel) this.G4M3Screen.getRewards(MainGame.reward_wheel);
...
class Shooter extends Phaser.Scene {
    ...
    initPauseWindow() {
        ...
        MainGame.updateTextWidth(buttonMuteSound.text, 140);
        var buttonHome = new ButtonText(0, buttonMuteSound.y + 80, "ss_ui", "btn_buy_0000", this.showG4M3Screen, this, MainGame.GAME_TEXT.home);
